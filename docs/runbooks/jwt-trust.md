...
A4 now requires the exact customer scope and matching order owner for reads/decisions; canonical and compatibility operations share the authorized decision behavior. The retired shared-email-token endpoint returns 404 without business invocation. See [customer order access rollout](customer-order-access.md) for re-login, Postman and integration migration steps. Gateway/function and deployment configuration remain separately owned release dependencies.
