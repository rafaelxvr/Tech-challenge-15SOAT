...
The native shim contract also exercises Windows drive-qualified paths and paths with spaces through a child process, verifies exact argument/exit-code forwarding, and performs provider-free YAML decoding and synthetic-state reads. The shim reads the OS argument vector because `pwsh -File` otherwise splits the colon in `-chdir=C:\path` during parameter binding. For the permitted console expression, the guard removes the incoming pipe terminator before PowerShell adds its outgoing newline; sending a second blank line makes Terraform console return an empty result. Neither correction expands the allowed commands or fixture boundaries.
