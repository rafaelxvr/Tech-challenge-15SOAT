...
sequenceDiagram
  ...
  alt invalid, expired, replayed or losing concurrent consume
    ...
    alt identity missing, inactive, changed or lookup fails
      F-->>C: Failure with no token; challenge remains consumed
    else current identity accepted
