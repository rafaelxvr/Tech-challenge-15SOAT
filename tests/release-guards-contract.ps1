...
try {
    ...
    foreach ($environment in @('staging','production')) {
        ...
        # Staging uses the canonical APP namespace. Production retains its
        # separately reviewed application namespace; neither is activated.
        ...
        $otherNamespace = if ($environment -ceq 'staging') { 'application' } else { 'app' }
        foreach ($entry in @(
