...
function Assert-BootstrapScalarString([object]$Value, [string]$Field) {
    if ($Value -isnot [string] -or [string]::IsNullOrWhiteSpace($Value)) {
        throw "Bootstrap contract field '$Field' must be a non-empty scalar string."
    }
}

function Read-BootstrapReview([object]$Release, [string]$Account) {
    $review = $Release.bootstrapReview
    if ($review -isnot [pscustomobject]) { throw 'APP release must include an object bootstrapReview for a writer release.' }
    ...
    foreach ($field in @('environment','sourceCommit','databaseHost','caSha256')) { Assert-BootstrapScalarString $review.$field "bootstrapReview.$field" }
    Assert-BootstrapScalarString $Release.environment 'release.environment'
    Assert-BootstrapScalarString $Release.sourceCommit 'release.sourceCommit'
    if ($review.schemaVersion -isnot [long] -or $review.schemaVersion -ne 1 -or $review.environment -cne $Release.environment -or
    ...
    if ($review.master -isnot [pscustomobject]) {
    ...
    Assert-BootstrapScalarString $review.master.arn 'bootstrapReview.master.arn'
    Assert-BootstrapScalarString $review.master.versionId 'bootstrapReview.master.versionId'
    ...
    if ($review.roles -isnot [pscustomobject]) { throw 'bootstrapReview roles must be an object.' }
    ...
    foreach ($role in @('migration','app','auth','notification')) {
        ...
        if ($reference -isnot [pscustomobject]) { throw "bootstrapReview role '$role' must be an object." }
        Assert-BootstrapScalarString $reference.arn "bootstrapReview.roles.$role.arn"
        Assert-BootstrapScalarString $reference.versionId "bootstrapReview.roles.$role.versionId"
        if ((@($reference.PSObject.Properties.Name | Sort-Object) -join ',') -cne 'arn,versionId' -or
...
function Read-BootstrapReceipt([string]$ReceiptJson, [object]$Release, [string]$Account) {
    ...
    if ($receipt -isnot [pscustomobject]) { throw 'Bootstrap receipt must be a JSON object.' }
    Assert-BootstrapScalarString $receipt.environment 'receipt.environment'
    Assert-BootstrapScalarString $receipt.sourceCommit 'receipt.sourceCommit'
    ...
    if ($receipt.outputs -isnot [pscustomobject]) { throw 'Bootstrap receipt outputs must be a JSON object.' }
    ...
    foreach ($field in $expectedOutput) { Assert-BootstrapScalarString $receipt.outputs.$field "receipt.outputs.$field" }
    if ($receipt.outputs.schemaVersion -cne 'V8' -or
        $receipt.outputs.authViewVersion -cne 'V5' -or
        $receipt.outputs.recipientViewVersion -cne 'V7') {
